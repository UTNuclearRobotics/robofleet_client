...
class Type:
    ...
    *arrays*. Therefore, we simply ignore any array length specified by the
    ...
    def fbs_type(self, base_ns):
        """ Fully qualified Flatbuffers type (including array syntax) """
        full_name = self.fbs_type_name(base_ns)
        if self.is_array:
            return "[{}]".format(full_name)
        return full_name

    def is_defined(self, base_ns, defined_types):
        """ Determine whether this type has been defined for Flatbuffers """
        return self.fbs_type_name(base_ns) in defined_types
    
    def mark_defined(self, base_ns, defined_types):
        """ Add this type to defined types set """
        defined_types.add(self.fbs_type_name(base_ns))
...
def gen_table(msg_type, items, defined_types, base_ns):
    ...
    for k, v in items:
        ...
        if v.fbs_type(base_ns) not in scalar_types:
            attrs = " (required)"
         
        yield "  {}:{}{};".format(k, v.fbs_type(base_ns), attrs)
    yield "}"
...
def gen_constants_table(msg_type, base_ns):
    ...
    for c in spec.constants:
        t = Type(c.type)
        if t.fbs_type(base_ns) in primitive_types:
            # name is converted to lower case to avoid all-uppercase camelCase in, e.g. JS code
...
def gen_msg(msg_type, defined_types, base_ns, gen_enums, gen_constants):
    ...
    if msg_type.is_defined(base_ns, defined_types):
        raise RuntimeError('Type ' + msg_type.ros_type + ' is already defined.')
    msg_type.mark_defined(base_ns, defined_types)
    ...
    yield "namespace {};".format(msg_type.full_namespace(base_ns))
    for x in gen_table(msg_type, zip(keys, types), defined_types, base_ns):
        yield x

def generate_schema(msg_type_names, base_ns, gen_enums, gen_constants):
    ...
    for x in gen_support(base_ns):
        yield x
    for msg_type_name in msg_type_names:
        for x in gen_msg(Type(msg_type_name), defined_types, base_ns, gen_enums, gen_constants):
            yield x
