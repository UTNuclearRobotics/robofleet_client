...
def get_srv_search_path():
    ...
    for p in rospack.list():
      package_paths = rosmsg._get_package_paths(p, rospack)
      search_path[p] = [os.path.join(d, 'srv') for d in package_paths]
...
def get_srv_spec(srv_type_name, search_path=get_srv_search_path()):
    """ Get ROS MsgSpec for given message type """
    context = genmsg.MsgContext.create_default()
    search_path = get_srv_search_path()
