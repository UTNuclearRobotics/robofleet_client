...
class Type:
    ...
    def __init__(self, ros_type, base_ns):
        ...
        match = re.match(
            r"^(?P<type>(?:(?P<ns>[\w\/]+)\/)?(?P<name>\w+))(?P<array>\[(?P<size>\d*)\])?$", ros_type)
        if match is None:
...
def gen_constants_enums(msg_type, spec) -> str:
    ...
    # https://google.github.io/flatbuffers/flatbuffers_guide_writing_schema.html
    allowed_enum_types = {"int8", "int16", "int32",
    ...
    for c in spec.constants:
        if c.type in allowed_enum_types:
            ...
            print("Warning: skipped non-integral constant {}.{}".format(
                msg_type.fbs_type(), c.name), file=sys.stderr)
...
def gen_msg(msg_type: Type,
    ...
    if msg_type.is_defined(defined_types):
        raise RuntimeError('Type ' + msg_type.ros_type +
                           ' is already defined.')
    ...
    raise RuntimeError(
        "ROS Message or Service type {} not found. Ensure any necessary packages are on your path.".format(msg_type.ros_type))
...
if __name__ == "__main__":
    ...
    lines = generate_schema(ns.message_type, base_ns=ns.base_namespace,
                            gen_enums=ns.gen_enums, gen_constants=ns.gen_constants)
